...
def get_fit_regressor(X_cr_train, y_cr_train, X_cr_test, y_cr_test, context, columns=None):
  ...
  regressor = RandomForestRegressor(n_estimators=350, max_samples=0.4, max_features=1, oob_score=True, random_state=42)
...
def get_least_important_feature_by_pi(X_cr_train,y_cr_train, X_cr_test,y_cr_test,regressor, walk=1):
  ...
  permutation_importance_s,all_feat_imp_df,order_column = compute_permutation_importance(X_cr_test,y_cr_test,regressor,metric=mean_squared_error)
...
def get_least_important_feature_by_sp(X_cr_train,y_cr_train, X_cr_test,y_cr_test,regressor, walk=1):
  ...
  lime_explainer = lime.lime_tabular.LimeTabularExplainer(X_cr_train.values,
                                                        ...
                                                        ,random_state=123)
  sp_obj_cr = submodular_pick.SubmodularPick(lime_explainer,X_cr_test.values, regressor.predict, num_features=9,  num_exps_desired=100)
...
if __name__ == "__main__":
    ...
    wf= WalkForward( datetime.datetime.strptime('2011-01-01','%Y-%m-%d'), datetime.datetime.strptime('2018-01-01','%Y-%m-%d'),4,no_walks=3)
    ...
    for idx, train_set,test_set in wf.get_walks():
        ...
        for ticker in tickers:
            ...
            if (len(X_cr_train)==0 or len(y_cr_test)==0):
                continue
            print('{0} train {1} {2}'.format(tiker,X_cr_train.index.min(),X_cr_train.index.max()))
            print('{0} test {1} {2}'.format(ticker,X_cr_test.index.min(),X_cr_test.index.max()))
            
            baseline,b_y_cr_test = get_fit_regressor(X_cr_train,y_cr_train,X_cr_test,y_cr_test,context={'walk':idx,'ticker':ticker,'method':'baseline','start':train_set.start,'end':train_set.end})
            ...
            for method,func in methods.items():
                columns,missing_col_median,missing_col_mean = func(X_cr_train,y_cr_train, X_cr_test,y_cr_test,baseline,walk=idx )
                ...
                missing_columns=missing_columns.append({'walk':idx,'method':method,'ticker':ticker,'removed_column_median':missing_col_median,'removed_column_mean':missing_col_mean},ignore_index=True)
