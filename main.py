...
def get_fit_regressor(X_cr_train, y_cr_train, X_cr_test, y_cr_test, context, columns=None, get_cross_validation_results=True):
  ...
  regressor = RandomForestRegressor(n_estimators=500, min_samples_leaf=5, max_features=1, oob_score=True, random_state=42)
...
def get_least_important_feature_by_fi(X_cr_train,y_cr_train, X_cr_test,y_cr_test,regressor, walk=1,features_no=1):
  ...
  feat_imp_s = pd.DataFrame({'features': all_columns, "feature_importance": regressor.feature_importances_}).sort_values('feature_importance', ascending=False)
...
def get_least_important_feature_by_pi(X_cr_train,y_cr_train, X_cr_test,y_cr_test,regressor, walk=1,features_no=1):
  ...
  permutation_importance_s,_,_ = compute_permutation_importance(X_cr_test,y_cr_test,regressor,metric=mean_squared_error)
...
def get_least_important_feature_by_sp(X_cr_train,y_cr_train, X_cr_test,y_cr_test,regressor, walk=1,features_no=1):
  ...
  lime_explainer = lime.lime_tabular.LimeTabularExplainer(X_cr_train.values,
                                                        ...
                                                        ,random_state=123)
  sp_obj_cr = submodular_pick.SubmodularPick(lime_explainer,X_cr_test.values, regressor.predict, num_features=9,  num_exps_desired=126)
  ...
  ranked_features = pd.concat([rank_w_s_median,rank_w_s_mean],axis=1).sort_values(by=['median_rank','mean_rank'],ascending=[False,False])
...
def get_cumulative_returns(data:pd.DataFrame,ticker):
  ...
  for i in indices:
      ...
      dummy_ret = pd.DataFrame((data['Close'].shift(1) - data['Open'].shift((i))) / data['Open'].shift((i)),columns=[columns_name])
...
if __name__ == "__main__":
    ...
    wf= WalkForward( datetime.datetime.strptime('2011-01-01','%Y-%m-%d'), datetime.datetime.strptime('2018-01-01','%Y-%m-%d'),4,no_walks=1)
    ...
    for idx, train_set,test_set in wf.get_walks():
        ...
        for ticker in tickers[:2]:
            ...
            baseline,b_y_cr_test,score = get_fit_regressor(X_cr_train,y_cr_train,X_cr_test,y_cr_test,context={'walk':idx,'ticker':ticker,'method':'baseline','start':train_set.start,'end':train_set.end},get_cross_validation_results=True)
            ...
            for method,func in methods.items():
                columns, missing_col_median, missing_col_mean = func(X_cr_train,y_cr_train, X_cr_test,y_cr_test,baseline,walk=idx,features_no=features_no )
                looc_fi_regressor, looc_y_cr_test, score_looc = get_fit_regressor(X_cr_train,y_cr_train,X_cr_test,y_cr_test,
                                                                        context={'walk':idx,'ticker':ticker,'method':method,'start':train_set.start,'end':train_set.end}, columns=columns)
